...
creator.create("FitnessTarget", base.Fitness, weights=(1.0, -1.0,))
...
def evalOneMax(individual):
    ...
    for i, vec in enumerate(individual):
        ...
        r1[i] = pearsonMatrix(v).sum()  # Pearson
        r2[i] = hammingMatrix(v).sum()  # Hamming

        # update_plots(WT_voltage, v, ax)
        # print(f'{score:8.5} -> [{vec[0]:3.5}, {vec[1]:3.5}, {vec[2]:3.5}]')
        with open(log_filename + '.csv', 'a') as f_log:
            f_log.write(f'{r1[i]},{r2[i]},{vec[0]}, {vec[1]}, {vec[2]} \n')
        sys.stdout.flush()
...
toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=0.0001, indpb=0.3)
