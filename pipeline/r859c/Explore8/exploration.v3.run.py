...
def scatter3d(x, y, z, cs, colorsMap='gist_ncar', title=None, filename=None, path=None):
    ...
    cbar.set_label('Accuracy in %')  # , rotation=270)
    ax.set_xlabel('gl')
    ax.set_ylabel('gK')
    ax.set_zlabel('gNa')
...
def Four_scatter3d(figs, filename=None, path=None, animation=False):  # colorsMap='jet'
    # Dividing figure
    x, y = [2, 2]
...
for Prog_part in Prog_parts:
    if Prog_part == 1:
        ...
        parm_min = [Orginal_MT_param[0] / 10, # smaller
                    Orginal_MT_param[1] / 10,
                    Orginal_MT_param[2] / 10]  # fields[0], fields[1], ....
        parm_max = [Orginal_MT_param[0] * 10, # smaller
                    Orginal_MT_param[1] * 10,
                    Orginal_MT_param[2] * 10]  # fields[0], fields[1], ....
        ...
        #     P.append(Process(target=test_parameters, args=(parm_min, parm_max, num_of_tests, crateria_cell_type,)))
        #     P[-1].start()
        # for p in range(num_process):
        #     P[p].join()
    ...
    if Prog_part == 3:
        ...
        slice = [100, 80, 60, 40, 30, 20, 10, 5, 2, -1]
        for s in tqdm(slice):
            ...
            for k1 in db.keys():
                        ...
                        if db[k1][k2][k3].shape[1] != 0:
                            if s > 0:
                                figs.append([
                                    ...
                                    'jet',
                                    k1 + " cell - " + k2 + " distance close to " + k3 + " - " + str(s) + " Samples"
                                ])
                            else:
                ...
                Four_scatter3d(figs, filename=k1, path=new_path, animation=True)
    ...
    if Prog_part == 5:
        ...
        path_org = './'
        path_org = os.path.dirname(os.path.abspath(__file__)) + '/'
        file = 'sumary_explor_result.100K.pbz2'
        num_of_candidates = 4
        ...
        for k1 in db.keys():  # messurment category
            if k1.__contains__('std') or k1.__contains__('avg'):
                continue
            l_min = np.where(db[k1]['MT']['WT'][:, 3] == min(db[k1]['MT']['WT'][:, 3]))[0]
            if num_of_candidates > l_min.shape[0]:
                l_min = l_min[:num_of_candidates]

            l_min_alt = np.where(db[k1]['WT']['WT'][:, 3] == min(db[k1]['WT']['WT'][:, 3]))[0]
            if num_of_candidates > l_min_alt.shape[0]:
                l_min_alt = l_min_alt[:num_of_candidates]

            delta[k1] = list()
            for phase in range(2):
                for i, d_point_from in enumerate(l_min):  # data point
                    for j, d_point_to in enumerate(l_min_alt):  # data point
                        if phase == 0:
                            delta[k1].append(
                                db[k1]['MT']['WT'][d_point_from, 0:3] - db[k1]['WT']['WT'][d_point_to, 0:3])
                        else:
                            delta[k1].append(
                                db[k1]['WT']['WT'][d_point_to, 0:3] - db[k1]['MT']['WT'][d_point_from, 0:3])
            delta[k1] = np.array(delta[k1], dtype=np.float)
            ...
            '''
            score_result = {}
            for k1_i, k1 in enumerate(delta.keys()):  # messurment category
                tqdm.write(f'Working on {k1}')

                l_min = np.where(db[k1]['MT']['MT'][:, 3] == min(db[k1]['MT']['MT'][:, 3]))[0]
                if num_of_candidates > l_min.shape[0]:
                    num_of_candidates = l_min.shape[0]

                pbar = tqdm()
                pbar.refresh()
                if num_of_candidates == 0 or delta[k1].shape[0] == 0:
                    tqdm.write(f'Category {k1} is empty')
                    continue

                pbar.reset(total=num_of_candidates * delta[k1].shape[0] * 2)
                score_result[k1] = list()
                for d_point_from in range(num_of_candidates):  # data point
                    for d_delta_a in range(delta[k1].shape[0]):
                        for phase in range(2):
                            if phase == 0:
                                param = db[k1]['MT']['MT'][l_min[d_point_from], 0:3] + delta[k1][d_delta_a, :]
                            else:
                                param = db[k1]['MT']['MT'][l_min[d_point_from], 0:3] - delta[k1][d_delta_a, :]
                            if sum(param < 0) == 0:
                                ...
                                score_result[k1].append(readline)
                            pbar.update()
                score_result[k1] = np.array(score_result[k1], dtype=np.float)
        ...
        for k1 in score_result.keys():  # messurment category
            Max = max(db[k1]['MT']['MT'][-1, 3], db[k1]['MT']['WT'][-1, 3])
            if Max < max(score_result[k1][:, 3]):
                Max = max(score_result[k1][:, 3])
            scatter3d(
                score_result[k1][:, 0], score_result[k1][:, 1], score_result[k1][:, 2],
                (1 - score_result[k1][:, 3] / Max) * 100,
                title=f'Fixed Mutation neuron - type {k1}',
                filename=f'- Fixed Mutation neuron - type {k1}',
                path=path
            )
            plt.close()
            # plot voltage curve 2 examples from every bin
            _, _, plot = plt.hist(
                (1 - score_result[k1][:, 3] / Max) * 100, bins=100,
                weights=(np.ones(score_result[k1][:, 3].shape[0]) / score_result[k1][:, 3].shape[0])
            )
            # add Original MT score
            t_coordinate = (1 - abs(observation_summary_statistics_WT['Spike Count'] -
                                    observation_summary_statistics_MT['Spike Count']).sum() / Max) * 100
            ...
            plt.title(f'Accuracy distribution\nTotal {score_result[k1][:, 3].shape[0]}')
            plt.xlabel("similarity to wild-type")
            plt.ylabel("% of models")

            plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
            plt.savefig(fname=path + '/' + f'histogram - Fixed Mutation neuron - type  {k1}', )
            plt.close()

            n, b = np.histogram(score_result[k1][:, 3], bins=100)
            for ex_bi, ex_b in enumerate(b):
                if ex_bi < n.shape[0] and n[ex_bi] > 0:
                    if ex_bi < b.shape[0] - 1:
                        clist = np.where(
                            (score_result[k1][:, 3] >= b[ex_bi]) * (score_result[k1][:, 3] <= b[ex_bi + 1]))
                    else:
                        clist = np.where(score_result[k1][:, 3] <= ex_b)

                    for i, l in enumerate(clist[0]):
                        if i > 1:
                            break
                        filename = f'Fixed Mutation neuron from {k1} group, b#{ex_bi} ' \
                                   f'#{i} score={score_result[k1][l, 3]}'
                        plots(WT_voltage, runMT(score_result[k1][l, 0:3]), filename=filename, path=path)
