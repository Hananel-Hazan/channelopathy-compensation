...
def calculate_summary_statistics(x):
    ...
    returnVec = np.zeros((x.shape[0], n_summary + 1))
    for index in range(x.shape[0]):
        ...
        # Pearson correlation
        ans_pearson = pearson([x[index, :], WT_voltage[index, :]])
...
def evalOneMax(individual):
    # r = test_parameters(individual)
    global count
    count += 1

    individual = [individual]
    r = np.zeros(len(individual))
    for i, vec in enumerate(individual):

        if 0 > vec[0] or vec[0] > 1:
            return np.iinfo(np.int32).max,
        if 0 > vec[1] or vec[1] > 1:
            return np.iinfo(np.int32).max,
        if 0 > vec[2] or vec[2] > 1:
            return np.iinfo(np.int32).max,
        ...
        # score = pearsonMatrix(v).sum()  # Pearson
        # score = np.sum(np.abs(WT_voltage - v)).sum()  # Hamming
        score = 0
        for ii,volt in enumerate(v):
            score += distance.hamming(WT_voltage[ii], volt)  # Hamming
        r[i] = score
